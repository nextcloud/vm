...
do
    case "$choice" in
        Calendar)
            wget -q "$STATIC"/calendar.sh -P "$SCRIPTS" && install_3rdparty_app calendar
        ;;
        Contacts)
            wget -q "$STATIC"/contacts.sh -P "$SCRIPTS" && install_3rdparty_app contacts
        ;;
        Webmin)
            wget -q "$STATIC"/webmin.sh -P "$SCRIPTS" && install_3rdparty_app webmin
        ;;
        *)
        ;;
    esac
