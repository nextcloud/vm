...
if [ -z "$PROVISIONING" ]
then
    msg_box "This server is designed to run with two disks, one for OS and one for DATA. This will get you the best performance since the second disk is using ZFS which is a superior filesystem.
