...
do_external_usb() {
	HDUUID=$(blkid -o value -s UUID /dev/sda2)
	SPUUID=$(blkid -o value -s UUID /dev/sda1)
	device='/dev/sda'
	SDCARD='/dev/mmcblk0'
	ROOT_PROFILE='/root/.profile'

	whiptail --yesno "The SDCARD is used now, do you want to use an HD/SSD? (preferred, also better when powered externally)" 20 60 1
	whiptail --yesno "Verify that you only have 1 externally powered USB Harddrive/SSD plugged in" 20 60 1
...
echo "UUID=$SPUUID none swap sw 0 0" >> /etc/fstab
sync
partprobe

# Set cmdline.txt
mount /dev/mmcblk0p1 /mnt
sed -i 's|root=/dev/mmcblk0p2|root=$HDUUID|g' /mnt/cmdline.txt
umount /mnt

# External HD
sed -i 's|/dev/mmcblk0p2|UUID=$HDUUID|g' /etc/fstab # change ROOT device so the system will know which one to use as ROOT
...
mount /dev/sda2 /mnt
    ...
    } | whiptail --title "Progress" --gauge "Please wait while moving SD content to HD" 6 60 0
