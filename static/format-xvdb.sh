...
. <(curl -sL https://raw.githubusercontent.com/zalonic/vm/master/lib.sh)
...
if isDevMounted "/dev/xvdb";
then
msg_box "/dev/xvdb is mounted and need to be unmounted before you can run this script."
    exit 1
fi
...
if isMounted "/dev/xvdb1";
then
msg_box "/dev/xvdb1 is mounted and need to be unmounted before you can run this script."
    exit 1
fi

if isDevPartOfZFS "xvdb";
then
msg_box "/dev/xvdb is a member of a ZFS pool and needs to be removed from any zpool before you can run this script."
    exit 1
fi

# Get the name of the drive
XVDB=$(fdisk -l | grep xvdb | awk '{print $2}' | cut -d ":" -f1 | head -1)
if [ "$XVDB" != "/dev/xvdb" ]
then
msg_box "It seems like /dev/xvdb does not exist.
...
then
msg_box "Formatting $XVDB when you hit OK.
...
*** WARNING: ALL YOUR DATA WILL BE ERASED! ***"
    ...
    check_command wipefs -a -f "$XVDB"
    sleep 0.5
    check_command zpool create -f -o ashift=12 "$LABEL_" "$XVDB"
