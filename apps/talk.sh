...
NC_UPDATE=1 && TURN_INSTALL=1 source /var/scripts/main/lib.sh
unset NC_UPDATE
unset TURN_INSTALL
else
# shellcheck disable=2034,2059
true
# shellcheck source=lib.sh
NC_UPDATE=1 && TURN_INSTALL=1 . <(curl -sL https://raw.githubusercontent.com/nextcloud/vm/testing/lib.sh)
unset NC_UPDATE
unset TURN_INSTALL
fi
